...
public class TravellersCompassCloningRecipe extends CustomRecipe {
    public TravellersCompassCloningRecipe(ResourceLocation id, CraftingBookCategory category) {
        super(id, category);
    }
    ...
    public boolean matches(CraftingContainer container, Level level) {
        ...
        for(ItemStack itemStack : container.getItems()) {
            if (!itemStack.isEmpty())
                if (itemStack.is(PPItems.TRAVELLERS_COMPASS.get())) {
    ...
    public ItemStack assemble(CraftingContainer container, RegistryAccess registryAccess) {
        ...
        if (!compassToCopy.isEmpty() && compassToCopy.hasTag() && compassesToCreate >= 1) {
            ItemStack result = new ItemStack(PPItems.TRAVELLERS_COMPASS.get(), compassesToCreate);
