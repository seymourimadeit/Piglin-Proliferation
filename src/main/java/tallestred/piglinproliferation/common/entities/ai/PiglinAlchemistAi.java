...
public class PiglinAlchemistAi extends AbstractPiglinAi<PiglinAlchemist> {
    ...
    protected RunOne<PiglinAlchemist> idleMovementBehaviors() {
        return new RunOne<>(ImmutableList.of(Pair.of(MoveAroundPiglins.moveAroundPiglins(0.6F, true), 2), Pair.of(RandomStroll.stroll(0.6F), 2), Pair.of(InteractWith.of(EntityType.PIGLIN, 8, MemoryModuleType.INTERACTION_TARGET, 0.6F, 2), 2), Pair.of(BehaviorBuilder.triggerIf(PiglinAi::doesntSeeAnyPlayerHoldingLovedItem, SetWalkTargetFromLookTarget.create(0.6F, 3)), 2), Pair.of(new DoNothing(30, 60), 1)));
    }
        ...
        return new RunOne<>(ImmutableList.of(
                ...
                Pair.of(new ThrowPotionAtTargetTask<>(PotionContents.createItemStack(Items.SPLASH_POTION, Potions.STRONG_HEALING), LivingEntity::isAlive, (piglin) -> {
                    ...
                    if (!list.isEmpty()) {
                        for (AbstractPiglin piglin1 : list) {
                            if (piglin1.getTarget() != null || piglinAlchemist.getTarget() != null)
                                return piglin.isAlive() && piglin.getHealth() < piglin.getMaxHealth() && piglinAlchemist.beltInventory.noneMatch(stack -> potionContents(stack).potion().orElse(Potions.WATER) == Potions.STRONG_REGENERATION) && list.size() > 2;
                            /*
                            This makes it so alchemists don't attempt to throw a healing potion if there's only 2 or less of them,
                            as if they did it would make it so only one is attacking while the other is failing to throw the potion
                            because the attacker would just keep pushing into them
                            */
                        }
                    }
                    return piglin.isAlive() && piglin.getHealth() < piglin.getMaxHealth();
                ...
                Pair.of(new ThrowPotionAtSelfTask<>(PotionContents.createItemStack(Items.SPLASH_POTION, Potions.STRONG_HEALING),
                        (alchemist) -> alchemist.isAlive() && alchemist.getHealth() < alchemist.getMaxHealth() && alchemist.beltInventory.noneMatch(stack -> potionContents(stack).potion().orElse(Potions.WATER) == Potions.STRONG_REGENERATION)), 2)));
    }
