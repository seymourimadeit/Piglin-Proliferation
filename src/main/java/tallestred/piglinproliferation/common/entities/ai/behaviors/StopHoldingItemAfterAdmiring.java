...
public class StopHoldingItemAfterAdmiring<E extends Piglin> extends StopHoldingItemIfNoLongerAdmiring {
    public static BehaviorControl<Piglin> create(AbstractPiglinAi aiInstance, ResourceLocation lootTableLocation, ResourceLocation cheap, ResourceLocation expensive) {
                ...
                return (p_259681_, piglin, p_259451_) -> {
                    if (!piglin.getOffhandItem().isEmpty() && !piglin.getOffhandItem().canPerformAction(net.minecraftforge.common.ToolActions.SHIELD_BLOCK)) {
                        if (!ModList.get().isLoaded("sapience"))
                            ...
                            ModCompat.stopHoldingOffHandItem(piglin, true, cheap, expensive);
                        return true;
                    } else {
                        return false;
                    }
