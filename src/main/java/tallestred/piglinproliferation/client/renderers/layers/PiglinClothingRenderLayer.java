...
public class PiglinClothingRenderLayer<T extends ZombifiedPiglin, M extends PiglinModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTicks, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        if (!PPConfig.CLIENT.ziglinTextures.get())
            return;
        if (getClothingTexture(pLivingEntity) != null && !pLivingEntity.getData(PPCapabilities.TRANSFORMATION_TRACKER).isEmpty())
    ...
    protected ResourceLocation getClothingTexture(T livingEntity) {
        String texture = "textures/entity/piglin/clothing_" + livingEntity.getData(PPCapabilities.TRANSFORMATION_TRACKER) + ".png";
