...
public class PPClientEvents {
    ...
    public static void onEntityRenderPost(RenderLivingEvent.Post<LivingEntity, EntityModel<LivingEntity>> event) {
        ...
        if (BucklerItem.getChargeTicks(PPItems.checkEachHandForBuckler(entityIn)) > 0) {
            if (!PPConfig.CLIENT.RenderAfterImage.get()) return;
            for (int i = 0; i < 5; i++) {
                if (i != 0) {
                    stack.pushPose();
                    ...
                    stack.translate(0.0D, -1.501F, i * motionZ * 4 / BucklerItem.getChargeTicks(PPItems.checkEachHandForBuckler(entityIn)));
                    float f8 = 0.0F;
                    float f5 = 0.0F;
                    ...
                    RenderType rendertype = PPClientEvents.getRenderType(entityIn, renderer, model, flag, flag1, flag2);
                    if (rendertype != null) {
                    ...
                    if (!entityIn.isSpectator()) {
                        if (PPConfig.CLIENT.RenderEntityLayersDuringAfterImage.get()) {
